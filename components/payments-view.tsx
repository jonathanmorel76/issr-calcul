...
export default function PaymentsView({userId}:{userId:string}){
 ...
 function useAnalysis(period?:PayslipAnalysis['periodSuggestions'][number]){
  if(!analysis)return
  ...
  setNote(v=>v||`Montant proposé par ${methodLabel(analysis.method).toLowerCase()} · ${confidenceLabel(analysis.confidence).toLowerCase()}`)
 ...
 return <main className="dashboard-main reports-page payments-page">
  ...
  {loading?<div className="dashboard-panel report-loading-skeleton">Chargement du rapprochement…</div>:<>
   ...
   <section className="dashboard-panel report-panel"><div className="report-head"><div><span className="eyebrow">Lecture de la fiche de paie</span><h2>Pré-remplir depuis un bulletin</h2></div>{!premium&&<span className="tag">Premium</span>}</div><div className="mr-form"><label className="mr-wide">Fiche de paie à analyser<select value={documentId} onChange={e=>setDocumentId(e.target.value)} disabled={!premium}><option value="">Choisir une fiche de paie…</option>{docs.map(d=><option value={d.id} key={d.id}>{d.title||d.file_name}</option>)}</select><small>PDF texte, PDF scanné, JPG, PNG ou WebP. L’OCR peut être moins précis qu’un PDF texte.</small></label><button className="btn btn-export" onClick={analyzePayslip} disabled={!premium||!documentId||analysisBusy}>{analysisBusy?'Analyse OCR en cours…':'Analyser la fiche de paie'}</button></div>{analysisError&&<p className="mr-message">{analysisError}</p>}{analysis&&<div className="dashboard-panel"><div className="report-head"><div><span className="eyebrow">{methodLabel(analysis.method)} · {confidenceLabel(analysis.confidence)}</span><h3>{analysis.suggestions.length?`${analysis.suggestions.length} ligne(s) ISSR détectée(s)`:'Aucune ligne ISSR certaine détectée'}</h3></div><strong>{analysis.total>0?euro(analysis.total):'—'}</strong></div>{analysis.inferredPaymentMonth&&<p>Mois du bulletin probable : <strong>{monthLabel(analysis.inferredPaymentMonth)}</strong></p>}{analysis.periodSuggestions.length>1&&<div className="mr-list">{analysis.periodSuggestions.map((period,index)=><article key={`${period.entitlementMonth??'unknown'}-${index}`}><div><strong>{period.entitlementMonth?monthLabel(period.entitlementMonth):'Période non identifiée'}</strong><span>{euro(period.total)}</span><small>{period.lineCount} ligne(s) rapprochée(s)</small></div><button className="btn btn-export" onClick={()=>useAnalysis(period)}>Utiliser</button></article>)}</div>}{analysis.suggestions.length>0&&<details className="indemnity-manual"><summary><span><strong>Voir les lignes détectées</strong><small>À contrôler avant validation</small></span></summary><div className="mr-list">{analysis.suggestions.map((item,index)=><article key={`${item.label}-${index}`}><div><strong>{euro(item.amount)}</strong><span>{item.entitlementMonth?monthLabel(item.entitlementMonth):'Période non identifiée'}</span><small>{item.label}</small></div></article>)}</div></details>}<p><small>{analysis.disclaimer}</small></p>{analysis.periodSuggestions.length<=1&&<button className="btn btn-primary" onClick={()=>useAnalysis(analysis.periodSuggestions[0])} disabled={!analysis.suggestions.length}>Utiliser cette proposition</button>}</div>}</section>
