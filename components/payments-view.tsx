...
type PayslipAnalysis={document:{id:string;title:string|null;fileName:string};inferredPaymentMonth:string|null;suggestions:{label:string;amount:number}[];total:number;confidence:'low'|'medium'|'high';disclaimer:string}
...
export default function PaymentsView({userId}:{userId:string}){
 ...
 function useAnalysis(){
  ...
  setReceived(analysis.total>0?analysis.total.toFixed(2).replace('.',','):'')
  setNote(v=>v||`Montant proposé par lecture automatique de la fiche de paie · ${confidenceLabel(analysis.confidence).toLowerCase()}`)
  setMessage('Proposition reportée dans le formulaire. Vérifiez-la avant d’enregistrer le versement.')
 }
 async function save(){
 ...
 return <main className="dashboard-main reports-page payments-page">
  ...
  {loading?<div className="dashboard-panel report-loading-skeleton">Chargement du rapprochement…</div>:<>
   ...
   <section className="dashboard-panel report-panel"><div className="report-head"><div><span className="eyebrow">Lecture de la fiche de paie</span><h2>Pré-remplir depuis un bulletin PDF</h2></div>{!premium&&<span className="tag">Premium</span>}</div><div className="mr-form"><label className="mr-wide">Fiche de paie à analyser<select value={documentId} onChange={e=>setDocumentId(e.target.value)} disabled={!premium}><option value="">Choisir une fiche de paie…</option>{docs.map(d=><option value={d.id} key={d.id}>{d.title||d.file_name}</option>)}</select><small>V1 : PDF contenant du texte. Les scans et photos nécessiteront l’OCR.</small></label><button className="btn btn-export" onClick={analyzePayslip} disabled={!premium||!documentId||analysisBusy}>{analysisBusy?'Analyse…':'Analyser la fiche de paie'}</button></div>{analysisError&&<p className="mr-message">{analysisError}</p>}{analysis&&<div className="dashboard-panel"><div className="report-head"><div><span className="eyebrow">Proposition · {confidenceLabel(analysis.confidence)}</span><h3>{analysis.suggestions.length?`${analysis.suggestions.length} ligne(s) ISSR détectée(s)`:'Aucune ligne ISSR certaine détectée'}</h3></div><strong>{analysis.total>0?euro(analysis.total):'—'}</strong></div>{analysis.inferredPaymentMonth&&<p>Mois de paie probable : <strong>{monthLabel(analysis.inferredPaymentMonth)}</strong></p>}{analysis.suggestions.length>0&&<div className="mr-list">{analysis.suggestions.map((item,index)=><article key={`${item.label}-${index}`}><div><strong>{euro(item.amount)}</strong><small>{item.label}</small></div></article>)}</div>}<p><small>{analysis.disclaimer}</small></p><button className="btn btn-primary" onClick={useAnalysis} disabled={!analysis.suggestions.length}>Utiliser cette proposition</button></div>}</section>
