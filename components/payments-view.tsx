...
type PeriodStatus='exact'|'close'|'partial'|'excess'|'no-rights'|'unknown'
type PeriodSuggestion={entitlementMonth:string|null;total:number;lineCount:number;expected:number|null;alreadyAllocated:number;outstanding:number|null;gap:number|null;matchPercent:number|null;status:PeriodStatus;label:string}
type PayslipAnalysis={document:{id:string;title:string|null;fileName:string};method:'pdf-text'|'ocr-image'|'ocr-pdf';inferredPaymentMonth:string|null;suggestions:{label:string;amount:number;entitlementMonth:string|null}[];periodSuggestions:PeriodSuggestion[];total:number;confidence:'low'|'medium'|'high';comparisonSummary?:string;disclaimer:string}
...
function confidenceLabel(value:PayslipAnalysis['confidence']){return value==='high'?'Confiance élevée':value==='medium'?'Confiance moyenne':'À vérifier attentivement'}
function methodLabel(value:PayslipAnalysis['method']){return value==='pdf-text'?'Lecture du texte PDF':value==='ocr-image'?'OCR de l’image':'OCR du PDF scanné'}
function statusLabel(value:PeriodStatus){return value==='exact'?'✅ Correspondance exacte':value==='close'?'🟢 Correspondance très probable':value==='partial'?'🟠 Versement partiel probable':value==='excess'?'🔴 Montant supérieur':value==='no-rights'?'⚪ Aucun droit connu':'⚠️ Période à identifier'}
function directValidationAllowed(value:PeriodStatus){return value==='exact'||value==='close'||value==='partial'}
...
export default function PaymentsView({userId}:{userId:string}){
 ...
 useEffect(()=>{setAnalysis(null);setAnalysisError('')},[documentId])
 async function analyzePayslip(){
  if(!premium||!documentId)return
  setAnalysisBusy(true);setAnalysisError('');setAnalysis(null)
  try{
   const response=await fetch('/api/analyze-payslip',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({documentId})})
   const body=await response.json()
   if(!response.ok)throw new Error(body?.error||'Analyse impossible.')
   setAnalysis(body as PayslipAnalysis)
  }catch(error:any){setAnalysisError(error?.message||'Analyse impossible.')}
  finally{setAnalysisBusy(false)}
 }
 function useAnalysis(period?:PeriodSuggestion){
  if(!analysis)return
  if(analysis.inferredPaymentMonth)setPaymentMonth(analysis.inferredPaymentMonth)
  const amount=period?.total??analysis.total
  if(period?.entitlementMonth)setEntitlementMonth(period.entitlementMonth)
  setReceived(amount>0?amount.toFixed(2).replace('.',','):'')
  setNote(`Montant proposé par ${methodLabel(analysis.method).toLowerCase()} · ${period?statusLabel(period.status).replace(/^[^A-Za-zÀ-ÿ]+/,'').toLowerCase():confidenceLabel(analysis.confidence).toLowerCase()}`)
  setMessage('Proposition reportée dans le formulaire. Vérifiez la période et le montant avant d’enregistrer le versement.')
 }
 async function confirmPeriod(period:PeriodSuggestion){
  if(!analysis||!premium||!period.entitlementMonth)return
  if(!analysis.inferredPaymentMonth){useAnalysis(period);setMessage('Le mois du bulletin n’a pas pu être identifié automatiquement. Renseignez-le avant validation.');return}
  const entitlement=period.entitlementMonth
  const payMonth=analysis.inferredPaymentMonth
  const warning=period.status==='partial'?`Il s’agit d’un versement partiel : ${period.gap!==null?euro(Math.abs(period.gap)):'une somme'} resterait encore à rapprocher.\n\n`:''
  if(!window.confirm(`${warning}Enregistrer ${euro(period.total)} versés en ${monthLabel(payMonth)} pour les droits de ${monthLabel(entitlement)} ?`))return
  setBusy(true);setMessage('')
  const generatedNote=`Validation depuis l’analyse de fiche de paie · ${statusLabel(period.status).replace(/^[^A-Za-zÀ-ÿ]+/,'')}`
  const row={user_id:userId,payment_month:`${payMonth}-01`,entitlement_month:`${entitlement}-01`,received_amount:period.total,note:generatedNote,source_document_id:documentId}
  ...
  setEntitlementMonth(entitlement);setPaymentMonth(payMonth);setReceived(period.total.toFixed(2).replace('.',','));setNote(generatedNote)
  setAnalysis(v=>v?{...v,periodSuggestions:v.periodSuggestions.map(item=>item.entitlementMonth===entitlement?{...item,alreadyAllocated:item.alreadyAllocated+period.total,outstanding:item.outstanding===null?null:Math.max(0,item.outstanding-period.total),gap:item.outstanding===null?item.gap:period.total-Math.max(0,item.outstanding-period.total)}:item)}:v)
  setMessage(`Versement validé : ${euro(period.total)} rapprochés avec les droits de ${monthLabel(entitlement)}.`)
 }
 async function save(){
 ...
 return <main className="dashboard-main reports-page payments-page">
  ...
  {loading?<div className="dashboard-panel report-loading-skeleton">Chargement du rapprochement…</div>:<>
   ...
   <section className="dashboard-panel report-panel"><div className="report-head"><div><span className="eyebrow">Lecture de la fiche de paie</span><h2>Comparer le bulletin à mes droits</h2></div>{!premium&&<span className="tag">Premium</span>}</div><div className="mr-form"><label className="mr-wide">Fiche de paie à analyser<select value={documentId} onChange={e=>setDocumentId(e.target.value)} disabled={!premium}><option value="">Choisir une fiche de paie…</option>{docs.map(d=><option value={d.id} key={d.id}>{d.title||d.file_name}</option>)}</select><small>PDF texte, PDF scanné, JPG, PNG ou WebP. L’OCR peut être moins précis qu’un PDF texte.</small></label><button className="btn btn-export" onClick={analyzePayslip} disabled={!premium||!documentId||analysisBusy}>{analysisBusy?'Analyse OCR en cours…':'Analyser la fiche de paie'}</button></div>{analysisError&&<p className="mr-message">{analysisError}</p>}{analysis&&<div className="dashboard-panel"><div className="report-head"><div><span className="eyebrow">{methodLabel(analysis.method)} · {confidenceLabel(analysis.confidence)}</span><h3>{analysis.suggestions.length?`${analysis.suggestions.length} ligne(s) ISSR détectée(s)`:'Aucune ligne ISSR certaine détectée'}</h3></div><strong>{analysis.total>0?euro(analysis.total):'—'}</strong></div>{analysis.inferredPaymentMonth&&<p>Mois du bulletin probable : <strong>{monthLabel(analysis.inferredPaymentMonth)}</strong></p>}{analysis.periodSuggestions.length>0&&<div className="mr-list">{analysis.periodSuggestions.map((period,index)=><article key={`${period.entitlementMonth??'unknown'}-${index}`}><div><strong>{period.entitlementMonth?monthLabel(period.entitlementMonth):'Période non identifiée'} · {statusLabel(period.status)}</strong><span>Détecté : {euro(period.total)}{period.outstanding!==null?` · Reste attendu : ${euro(period.outstanding)}`:period.expected!==null?` · Droits connus : ${euro(period.expected)}`:''}</span><small>{period.matchPercent!==null?`Correspondance ${period.matchPercent.toLocaleString('fr-FR')} % · `:''}{period.label}</small></div><div className="document-actions">{directValidationAllowed(period.status)&&period.entitlementMonth?<button className="btn btn-primary" onClick={()=>confirmPeriod(period)} disabled={busy}>{busy?'Enregistrement…':period.status==='partial'?'Valider le versement partiel':'Valider et rapprocher'}</button>:<button className="btn btn-export" onClick={()=>useAnalysis(period)} disabled={!period.entitlementMonth}>Examiner</button>}</div></article>)}</div>}{analysis.suggestions.length>0&&<details className="indemnity-manual"><summary><span><strong>Voir les lignes détectées</strong><small>À contrôler avant validation</small></span></summary><div className="mr-list">{analysis.suggestions.map((item,index)=><article key={`${item.label}-${index}`}><div><strong>{euro(item.amount)}</strong><span>{item.entitlementMonth?monthLabel(item.entitlementMonth):'Période non identifiée'}</span><small>{item.label}</small></div></article>)}</div></details>}<p><small>{analysis.disclaimer}</small></p></div>}</section>
   <section className="dashboard-panel report-panel"><div className="report-head"><div><span className="eyebrow">Affectation manuelle</span><h2>Corriger ou compléter le rapprochement</h2></div>{!premium&&<span className="tag">Premium</span>}</div><div className="mr-form"><label>Mois du versement / bulletin<input type="month" value={paymentMonth} onChange={e=>setPaymentMonth(e.target.value)} disabled={!premium}/><small>Ex. novembre si les ISSR de septembre sont payées en novembre.</small></label><label>Montant affecté à {monthLabel(entitlementMonth)} (€)<input inputMode="decimal" value={received} onChange={e=>setReceived(e.target.value)} placeholder="Ex : 319,68" disabled={!premium}/></label><label>Fiche de paie associée<select value={documentId} onChange={e=>setDocumentId(e.target.value)} disabled={!premium}><option value="">Aucune fiche de paie</option>{docs.map(d=><option value={d.id} key={d.id}>{d.title||d.file_name}</option>)}</select></label><label className="mr-wide">Note<input value={note} onChange={e=>setNote(e.target.value)} placeholder="Ex : rappel ISSR septembre sur paie de novembre" disabled={!premium}/></label><button className="btn btn-primary" onClick={save} disabled={!premium||busy}>{busy?'Enregistrement…':currentAllocation?'Mettre à jour l’affectation':'Ajouter ce versement'}</button></div>{message&&<p className="mr-message">{message}</p>}</section>
