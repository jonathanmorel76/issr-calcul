...
export default function MobileAppDock(){
 ...
 const activateRoute=useCallback((key:string,href:string)=>{
  if(!premium&&(key==='payroll'||key==='reports')){window.dispatchEvent(new Event('mr-open-premium'));return}
  setActiveKey(key);scrollToTop();router.push(href,{scroll:true})
 },[premium,router,scrollToTop])
