...
export default function IssrApp({ userId }: { userId: string }) {
  ...
  const sums=useMemo(()=>filtered.reduce((a,e)=>({jours:a.jours+1,km:a.km+Number(e.mileage_allowance),rep:a.rep+Number(e.rep_bonus),repPlus:a.repPlus+Number(e.rep_plus_bonus),total:a.total+Number(e.total_amount)}),{jours:0,km:0,rep:0,repPlus:0,total:0}),[filtered])
  ...
  return <>
    ...
    <main className="main">
      ...
      {loading?<div className="loading-screen">Chargement des déplacements…</div>:filtered.length===0?<div className="empty-state">Aucun déplacement pour cette période.</div>:<><div className="table-wrapper"><table className="desktop-table"><thead><tr><th>N°</th><th>Date</th><th>Origine</th><th>Destination</th><th>Km</th><th>REP</th><th>REP+</th><th>Indem. km</th><th>Prime REP</th><th>Prime REP+</th><th>Total</th><th></th></tr></thead><tbody>{filtered.map((e,i)=><tr key={e.id}><td className="td-center">{i+1}</td><td>{fmtDate(e.travel_date)}</td><td>{e.origin}</td><td>{e.destination}</td><td className="td-km">{e.distance_km}</td><td className="td-center">{e.is_rep?<span className="tag tag-rep">REP</span>:'—'}</td><td className="td-center">{e.is_rep_plus?<span className="tag tag-repplus">REP+</span>:'—'}</td><td className="td-amount">{fmtEuro(Number(e.mileage_allowance))}</td><td className="td-amount">{Number(e.rep_bonus)>0?fmtEuro(Number(e.rep_bonus)):'—'}</td><td className="td-amount">{Number(e.rep_plus_bonus)>0?fmtEuro(Number(e.rep_plus_bonus)):'—'}</td><td className="td-total">{fmtEuro(Number(e.total_amount))}</td><td><button className="btn btn-danger" onClick={()=>removeEntry(e.id)}>✕</button></td></tr>)}</tbody></table></div><div className="mobile-list">{filtered.map(e=><article className="mobile-entry" key={e.id}><div className="mobile-entry-head"><strong>{fmtDate(e.travel_date)}</strong><button className="btn btn-danger" onClick={()=>removeEntry(e.id)}>✕</button></div><div className="mobile-entry-route">{e.origin}<br/>→ {e.destination}</div><div className="mobile-entry-meta"><span className="km-pill">{e.distance_km} km</span>{e.is_rep&&<span className="tag tag-rep">REP</span>}{e.is_rep_plus&&<span className="tag tag-repplus">REP+</span>}</div><div className="mobile-entry-total"><span>ISSR</span><span>{fmtEuro(Number(e.total_amount))}</span></div></article>)}</div></>}
