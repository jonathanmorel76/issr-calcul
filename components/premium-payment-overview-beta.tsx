...
export default function PremiumPaymentOverviewBeta(){
 ...
 return createPortal(<section className={`beta-payment-overview ${mode} ${scope}`} aria-label="Synthèse Premium des versements ISSR">
  <div className="beta-payment-overview-head"><div><span className="eyebrow">Suivi Premium</span><h2>{scope==='dashboard'?'Mes versements à surveiller':'Attendu / versé sur la période'}</h2></div><span className="beta-premium-badge">PREMIUM</span></div>
  {mode==='free'?<div className="beta-payment-overview-lock"><strong>Repérez les mois à vérifier sans ouvrir chaque détail</strong><p>Premium consolide les montants attendus et versés, puis signale les écarts qui méritent une vérification.</p></div>:<>
