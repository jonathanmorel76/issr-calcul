...
export default function SecondaryViewShell({active,title,description,children}:{active:'bilans'|'documents';title:string;description:string;children:React.ReactNode}){
 ...
 return <div className="product-shell"><header className="product-hero"><div className="product-hero-copy"><BrandLogo inverse/><div className="product-hero-text"><h1>{title}</h1><p>{description}</p></div></div></header><nav className="product-tabs">{tabs.map(([id,label,href])=><Link key={id} href={href} prefetch className={active===id?'active':''}>{label}</Link>)}</nav>{children}</div>
}
