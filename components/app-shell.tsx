...
export default function AppShell({userId,initialEntries,initialMissions,initialEstablishments,initialDefaultOrigin,rateSchedules}:Props){
 ...
 const nav=<nav className="product-tabs"><button className={view==='dashboard'?'active':''} onClick={()=>setView('dashboard')}>Tableau de bord</button><button className={view==='establishments'?'active':''} onClick={()=>setView('establishments')}>Mes établissements</button><button className={view==='missions'?'active':''} onClick={()=>setView('missions')}>Mes missions</button><button onClick={()=>setView('indemnities')}>Mes indemnités</button><button disabled>Mes bilans</button><button disabled>Mes documents</button></nav>
