...
type WidgetId='next-mission'|'attention'|'month-activity'|'indemnities'|'establishments'|'quick-actions'
type AttentionItem={title:string;detail:string;view:View;level:'warning'|'info'}
...
const DEFAULT_WIDGETS:WidgetId[]=['next-mission','attention','month-activity','indemnities','establishments','quick-actions']
...
const WIDGET_STORAGE_KEY='mr-dashboard-widgets-v2'
...
export default function AppShell({userId,initialEntries,initialMissions,initialEstablishments,initialDefaultOrigin,rateSchedules}:Props){
 ...
 const attentionItems=useMemo<AttentionItem[]>(()=>{
  ...
  if(!establishments.length)items.push({title:'Aucun établissement configuré',detail:'Ajoutez votre premier établissement pour pouvoir planifier une mission.',view:'establishments',level:'info'})
  const missingDistance=establishments.filter(e=>e.usual_distance_km==null).length
  if(missingDistance)items.push({title:`${missingDistance} établissement${missingDistance>1?'s':''} sans distance`,detail:'Complétez la distance de référence pour automatiser correctement l’estimation ISSR.',view:'establishments',level:'warning'})
  const orphanMissions=missions.filter(m=>m.status!=='cancelled'&&(!m.establishment_id||!m.issr_establishments)).length
  if(orphanMissions)items.push({title:`${orphanMissions} mission${orphanMissions>1?'s':''} à rattacher`,detail:'Un établissement lié à une mission est manquant ou n’est plus disponible.',view:'missions',level:'warning'})
  if(missions.length&&!defaultOrigin.trim())items.push({title:'Adresse de départ à compléter',detail:'Renseignez votre adresse habituelle avant de générer de nouvelles journées indemnisées.',view:'missions',level:'info'})
  return items
 },[establishments,missions,defaultOrigin])

 useEffect(()=>{
  try{
   const saved=window.localStorage.getItem(WIDGET_STORAGE_KEY)
   if(saved){const parsed=JSON.parse(saved) as WidgetId[];const valid=parsed.filter((x,i)=>DEFAULT_WIDGETS.includes(x)&&parsed.indexOf(x)===i);if(valid.length)setVisibleWidgets(valid)}
  }catch{}
 },[])
 useEffect(()=>{try{window.localStorage.setItem(WIDGET_STORAGE_KEY,JSON.stringify(visibleWidgets))}catch{}},[visibleWidgets])
 useEffect(()=>{
 ...
 function moveWidget(id:WidgetId,direction:-1|1){setVisibleWidgets(v=>{const index=v.indexOf(id);const target=index+direction;if(index<0||target<0||target>=v.length)return v;const next=[...v];[next[index],next[target]]=[next[target],next[index]];return next})}
 function resetWidgets(){setVisibleWidgets(DEFAULT_WIDGETS)}
 ...
 {customizing&&<section className="widget-customizer"><div className="widget-customizer-copy"><strong>Personnaliser mon tableau de bord</strong><p>Affichez uniquement les widgets utiles et réorganisez-les selon vos priorités. L’ordre est enregistré sur cet appareil.</p><button className="widget-reset" onClick={resetWidgets}>Réinitialiser l’affichage</button></div><div className="widget-toggles">{DEFAULT_WIDGETS.map(id=>{const active=visibleWidgets.includes(id),index=visibleWidgets.indexOf(id);return <div className={`widget-toggle-row ${active?'active':''}`} key={id}><label><input type="checkbox" checked={active} onChange={()=>toggleWidget(id)}/><span>{WIDGET_LABELS[id]}</span></label><div className="widget-order-actions"><button aria-label={`Monter ${WIDGET_LABELS[id]}`} disabled={!active||index<=0} onClick={()=>moveWidget(id,-1)}>↑</button><button aria-label={`Descendre ${WIDGET_LABELS[id]}`} disabled={!active||index<0||index>=visibleWidgets.length-1} onClick={()=>moveWidget(id,1)}>↓</button></div></div>})}</div></section>}
 <section className="widget-grid">{visibleWidgets.map(widgetContent)}</section>
 </main></div>
}
