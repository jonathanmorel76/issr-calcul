...
export default function PremiumOperationalInsightsBeta(){
 ...
 return createPortal(<section className={`beta-operational-insights ${scope} ${mode}`} aria-label={scope==='establishments'?'Analyse des établissements':'Contrôles des missions'}>
  <div className="beta-operational-head"><div><span className="eyebrow">Suivi</span><h2>{scope==='establishments'?'Analyse par établissement':'Contrôles de cohérence'}</h2></div><span className="beta-premium-badge">PREMIUM</span></div>
  {loading?<div className="beta-operational-loading">Analyse en cours…</div>:mode==='free'?<div className="beta-operational-lock"><strong>{scope==='establishments'?'Comprenez où vous remplacez le plus':'Repérez les missions à vérifier automatiquement'}</strong><p>{scope==='establishments'?'Visualisez en un coup d’œil les jours, kilomètres, missions et indemnités estimées pour chaque établissement.':'Repérez les chevauchements et les informations manquantes qui peuvent fragiliser votre suivi.'}</p></div>:scope==='establishments'?<>
