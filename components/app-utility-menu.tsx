...
export default function AppUtilityMenu(){
 ...
 const [displayName,setDisplayName]=useState(''),[birthDate,setBirthDate]=useState(''),[avatarKind,setAvatarKind]=useState<AvatarKind>('teacher_male'),[photoFile,setPhotoFile]=useState<File|null>(null)
 ...
 useEffect(()=>{if(profile){setDisplayName(profile.display_name??'');setBirthDate(profile.birth_date??'');setAvatarKind(profile.avatar_kind??'teacher_male')}},[profile])
 ...
 async function saveProfile(){
  ...
  try{
   ...
   const row:UserProfile={user_id:userId,display_name:displayName.trim()||null,birth_date:birthDate||null,avatar_kind:photoFile?'photo':avatarKind,avatar_path:avatarPath}
   const {data,error}=await supabase.from('issr_profiles').upsert(row,{onConflict:'user_id'}).select('user_id,display_name,birth_date,avatar_kind,avatar_path').single()
 ...
 return <><button className="app-burger" aria-label="Ouvrir le menu" aria-expanded={open} onClick={()=>setOpen(true)}><span/><span/><span/></button>{open&&<div className="app-menu-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><aside className="app-menu-drawer" role="dialog" aria-modal="true" aria-label="Menu Mon Remplacement"><div className="app-menu-head"><div><strong>{view==='home'?'Menu':view==='rates'?'Grille tarifaire':view==='sources'?'Sources officielles':view==='method'?'Méthode de calcul':'Mon compte'}</strong><small>Mon Remplacement</small></div><button className="app-menu-close" onClick={()=>setOpen(false)} aria-label="Fermer">×</button></div>{view!=='home'&&<button className="app-menu-back" onClick={()=>navigate('home')}>← Retour au menu</button>}
 {view==='home'&&<div className="app-menu-home"><section><span className="app-menu-section-label">Références</span><button onClick={()=>navigate('rates')}><span>€</span><div><strong>Grille tarifaire ISSR</strong><small>Barème actuellement appliqué et tranches kilométriques</small></div><b>›</b></button><button onClick={()=>navigate('sources')}><span>↗</span><div><strong>Sources officielles</strong><small>Textes, références et date de vérification</small></div><b>›</b></button><button onClick={()=>navigate('method')}><span>∑</span><div><strong>Méthode de calcul</strong><small>Comprendre distance, ISSR et primes</small></div><b>›</b></button></section><section><span className="app-menu-section-label">Compte</span><button onClick={()=>navigate('account')}><span>◉</span><div><strong>Mon compte</strong><small>Profil, avatar, anniversaire et sécurité</small></div><b>›</b></button></section></div>}
