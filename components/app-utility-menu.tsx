...
import { TEACHER_STATUS_OPTIONS, teacherStatusLabel, type TeacherStatus } from '@/lib/teacher-status'
...
export default function AppUtilityMenu(){
 ...
 const [displayName,setDisplayName]=useState(''),[birthDate,setBirthDate]=useState(''),[teachingStartDate,setTeachingStartDate]=useState(''),[teacherStatus,setTeacherStatus]=useState<TeacherStatus|''>(''),[avatarKind,setAvatarKind]=useState<AvatarKind>('teacher_male'),[photoFile,setPhotoFile]=useState<File|null>(null)
 ...
 useEffect(()=>{if(profile){setDisplayName(profile.display_name??'');setBirthDate(profile.birth_date??'');setTeachingStartDate(profile.teaching_start_date??'');setTeacherStatus(profile.teacher_status??'');setAvatarKind(profile.avatar_kind??'teacher_male')}},[profile])
 ...
 const statusLabel=teacherStatusLabel(teacherStatus,true)
 ...
 async function saveProfile(){
  ...
  try{
   ...
   const row:UserProfile={user_id:userId,display_name:displayName.trim()||null,birth_date:birthDate||null,teaching_start_date:teachingStartDate||null,teacher_status:teacherStatus||null,avatar_kind:photoFile?'photo':avatarKind,avatar_path:avatarPath}
   const {data,error}=await supabase.from('issr_profiles').upsert(row,{onConflict:'user_id'}).select('user_id,display_name,birth_date,teaching_start_date,teacher_status,avatar_kind,avatar_path').single()
