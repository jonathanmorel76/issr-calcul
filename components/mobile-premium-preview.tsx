...
const COPY:Record<ViewKey,Copy>={
 dashboard:{title:'Votre suivi, sans zone d’ombre',free:'Mission en cours, météo et estimation ISSR restent disponibles en accès libre.',premium:'Ajoutez les alertes, tendances et contrôles de versement à votre tableau de bord.',benefits:['Alertes et points à vérifier','Évolution de vos indemnités','Synthèse attendu / versé']},
 establishments:{title:'Comprendre vos établissements',free:'Créer, modifier et retrouver vos établissements reste disponible en accès libre.',premium:'Identifiez les écoles où vous intervenez le plus et ce qu’elles représentent dans votre activité.',benefits:['Jours effectués par établissement','Kilomètres cumulés','ISSR estimées par lieu']},
 missions:{title:'Sécuriser le suivi de vos missions',free:'Créer vos missions et conserver votre historique reste disponible en accès libre.',premium:'Repérez automatiquement les situations qui méritent une vérification.',benefits:['Chevauchements de périodes','Informations manquantes','Situations inhabituelles à vérifier']},
 indemnities:{title:'Vérifier ce qui vous est versé',free:'Le calcul de ce que vous devriez recevoir reste disponible en accès libre.',premium:'Comparez vos estimations aux versements réellement reçus et retrouvez rapidement les écarts.',benefits:['Montant réellement reçu','Attendu vs versé','Mois à vérifier']},
 payroll:{title:'La fonctionnalité phare : contrôler votre paie',free:'Vos droits ISSR restent calculés en accès libre.',premium:'Rapprochez vos fiches de paie avec les droits calculés, repérez les montants manquants et préparez une régularisation depuis une seule vue.',benefits:['Analyse des fiches de paie','Attendu vs réellement versé','Détection et priorisation des écarts','Dossier de régularisation']},
 reports:{title:'Prendre du recul sur votre année',free:'Le mois courant reste consultable en accès libre.',premium:'Accédez à l’année scolaire complète, aux analyses détaillées et aux exports.',benefits:['Année scolaire complète','Analyses et classements','Exports PDF et Excel']},
 documents:{title:'Centraliser vos justificatifs',free:'Conservez jusqu’à 5 documents essentiels dans votre espace en accès libre.',premium:'Classez davantage de documents et reliez-les directement à votre activité.',benefits:['Archivage étendu','Association aux missions','Préparation à la vérification automatique']},
}
...
export default function MobilePremiumPreview(){
 ...
 return <>
  <button type="button" className={`beta-mode-pill ${mode}`} onClick={()=>setOpen(true)} aria-label="Ouvrir mon offre"><span className="beta-mode-dot"/><span>{premiumActive?'Premium':'Accès libre'}</span></button>
  {open&&<div className="beta-mode-backdrop" role="dialog" aria-modal="true" aria-label="Mon Remplacement Premium" onClick={()=>setOpen(false)}>
   <section className="beta-mode-sheet" onClick={e=>e.stopPropagation()}>
    ...
    <p className="beta-plan-intro">{premiumActive?'Toutes les fonctions Premium sont activées dans cette préversion.':'Gardez le calcul et l’organisation essentiels en accès libre. Passez à Premium pour analyser, vérifier et automatiser votre suivi.'}</p>
    {!premiumActive&&<><div className="beta-benefit-focus"><strong>{copy.title}</strong><p>{copy.premium}</p><ul>{copy.benefits.map(x=><li key={x}><span aria-hidden="true">✓</span>{x}</li>)}</ul></div><div className="beta-billing-choice" role="radiogroup" aria-label="Choisir la formule"><button type="button" className={billing==='annual'?'active':''} onClick={()=>setBilling('annual')} role="radio" aria-checked={billing==='annual'}><span className="beta-save-chip">−37 %</span><strong>Annuel</strong><b>29,99 € <small>/ an</small></b><em>soit 2,50 € / mois</em></button><button type="button" className={billing==='monthly'?'active':''} onClick={()=>setBilling('monthly')} role="radio" aria-checked={billing==='monthly'}><strong>Mensuel</strong><b>3,99 € <small>/ mois</small></b><em>sans engagement annuel</em></button></div><button type="button" className="beta-subscribe-cta" onClick={activate}>Démarrer mes 14 jours d’essai</button><p className="beta-trial-note">Aucun paiement n’est effectué dans cette Beta. Le bouton active simplement l’expérience Premium de démonstration.</p></>}
    ...
    <div className="beta-plan-footer"><button type="button" onClick={()=>setMode(premiumActive?'free':'premium')}>{premiumActive?'Revenir à l’accès libre':'Activer Premium pour la Beta'}</button><span>Simulation de préproduction</span></div>
   </section>
  </div>}
 </>
}
